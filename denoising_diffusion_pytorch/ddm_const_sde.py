...
class DDPM(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, up_scale=1, cond=None, denoise=True, mask=None):
    ...
    @torch.no_grad()
    def sample_given_y(self, batch_size=16, up_scale=1, cond=None, denoise=True, mask=None, y=None, mask_y=None):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        return self.sample_fn_given_y((batch_size, channels, image_size[0], image_size[1]),
                              up_scale=up_scale, unnormalize=True, cond=cond, denoise=denoise, y=y, mask_y=mask_y)
    
    @torch.no_grad()
    def sample_fn_given_y(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, y=None, mask_y=None):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], \
                                                                             self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        ...
        for i, time_step in enumerate(time_steps):
            ...
            # correct C
            x0 = self.pred_x0_from_xt(img, noise, C, cur_time)
            x0 = mask_y.to(x0.device) * y.to(x0.device) + (torch.ones_like(mask_y, dtype=x0.dtype, device=x0.device) - mask_y.to(x0.device)) * x0
            if self.clip_x_start:
                x0.clamp_(-1., 1.)
                # C.clamp_(-2., 2.)
            C = -1 * x0
    ...
    def sample_given_y_with_Tx(self, batch_size=16, up_scale=1, cond=None, denoise=True, mask=None, y=None, mask_y=None, path=None):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        return self.sample_fn_given_y_with_Tx((batch_size, channels, image_size[0], image_size[1]),
                              up_scale=up_scale, unnormalize=True, cond=cond, denoise=denoise, y=y, mask_y=mask_y, path=path)
    
    
    def sample_fn_given_y_with_Tx(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, y=None, mask_y=None, path=None):
        ...
        for i, time_step in enumerate(time_steps):
            ...
            # xx = torch.cat([
            #     # xxx, 
            #     torch.cat([xxx[:, :, :-5, :] - xxx[:, :, 5:, :], torch.zeros(xxx.shape[0], xxx.shape[1], 5, xxx.shape[3]).to(xxx.device)], dim=2),
            #     torch.cat([xxx[:, :, :, :-5] - xxx[:, :, :, 5:], torch.zeros(xxx.shape[0], xxx.shape[1], xxx.shape[2], 5).to(xxx.device)], dim=3)
            #     ], dim=1)
            # yy = torch.cat([
            #     # y, 
            #     torch.cat([y[:, :, :-5, :] - y[:, :, 5:, :], torch.zeros(y.shape[0], y.shape[1], 5, y.shape[3]).to(xxx.device)], dim=2),
            #     torch.cat([y[:, :, :, :-5] - y[:, :, :, 5:], torch.zeros(y.shape[0], y.shape[1], y.shape[2], 5).to(xxx.device)], dim=3)
            #     ], dim=1)
            # loss = nn.MSELoss()(torch.exp(2 * (xx * 0.5 + 0.5)), torch.exp(2 * (yy * 0.5 + 0.5)))
            loss = nn.MSELoss()(torch.exp(2 * ((mask_y * x0 * 0.5 + 0.5))), torch.exp(2 * ((y * 0.5 + 0.5))))
            cond.grad = None
            loss.backward()
            cond_grad = -cond.grad[0, 1:2]
            cond_grad[cond_grad<0] = 0
            cond_grad = (cond_grad - cond_grad.min()) / (cond_grad.max() - cond_grad.min())
            if path:
                import matplotlib.pyplot as plt
                import torchvision as tv
                
                tv.utils.save_image(cond_grad.cpu(), path + f"_step({i})_grad.png")
                # plt.imshow(cond_grad.cpu())
                # plt.savefig(path + f"_{i}_grad.png")
                # plt.clf()
            ...
            if path:
                # import matplotlib.pyplot as plt
                # import torchvision as tv
                tv.utils.save_image(x0.cpu()/2+0.5, path + f"_step({i})_x0.png")
            pos_now = torch.argmax(cond[0, 1])
            pos = torch.argmax(cond_grad)
            if loss < min_loss:
                best_pos = pos
                min_loss = loss
            # if min_loss < 0.02:
            #     threshod = cur_time ** (1/2)
            #     r = torch.rand(1)[0]
            #     if r < threshod:
            #         cond.requires_grad = False
            #         cond[0, 1, pos_now // cond.shape[-1], pos_now % cond.shape[-1]] = -1
            #         cond[0, 1, pos // cond.shape[-1], pos % cond.shape[-1]] = 1
            #         cond.requires_grad = True
            #     else:
            #         pos = best_pos
            #         cond.requires_grad = False
            ...
            cond.requires_grad = False
            cond[0, 1:2] += - cond.grad[0, 1:2]
            cond.requires_grad = True
            cond_grad_sum += - cond.grad[0, 1:2]

            if path:
                # import matplotlib.pyplot as plt
                # import torchvision as tv
                cgd = cond_grad_sum.clone()
                cgd[cgd<0] = 0
                cgd = (cgd - cgd.min()) / (cgd.max() - cgd.min())
                tv.utils.save_image(cgd.cpu(), path + f"_step({i})_Tx.png")
    ...
    def get_bs_from_xy(self, xy, size=256):
        ...
        pos_x = xy[:, 0].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, size, size)
        pos_y = xy[:, 1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, size, size)

        M = torch.exp(- ((R - pos_x)**2 + (C - pos_y)**2) / sigma)
        return M

    def get_quantized_bs_from_multi_xy(self, xy, size=256):
        # xy: [1, numTx, 2]
        # out: [1, 1, 256, 256]
        # sigma = 1
        device = xy.device
        ...
        M = -1 * torch.ones(batch_size, 1, size, size, dtype=torch.float32, device=device)

        for i in range(batch_size):
            for j in range(numTx):
                x, y = xy[i, j]
                x = torch.floor(torch.clip(x, 0, 255)).long()
                y = torch.floor(torch.clip(y, 0, 255)).long()
                M[i, 0, x, y] = 1

        return M
    

    def get_bs_from_multi_xy(self, xy, size=256):
        # xy: [1, numTx, 2]
        # out: [1, 1, 256, 256]
        sigma = 10
        device = xy.device
    ...
    def sample_given_y_MultiTx_BScorr_seenmean_SGDM_vqsg_heuristic(self, batch_size=16, up_scale=1, cond=None, denoise=True, mask=None, y=None, mask_y=None, path=None, bs=None, args=None):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        return self.sample_fn_given_y_MultiTx_BScorr_seenmean_SGDM_vqsg_heuristic((batch_size, channels, image_size[0], image_size[1]),
                              up_scale=up_scale, unnormalize=True, cond=cond, denoise=denoise, y=y, mask_y=mask_y, path=path, bs=bs, args=args)
    


    def sample_fn_given_y_MultiTx_BScorr_seenmean_SGDM_vqsg_heuristic(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, y=None, mask_y=None, path=None, bs=None, args=None):
        bs_start = bs.clone()
        bs_isnotbuilding = bs.clone()
        ...
        BS_m = torch.zeros((batch, bs.shape[1], 2), dtype=bs.dtype, device=device)
        for i, time_step in enumerate(time_steps):

            cond = cond.detach()
            BSS = self.get_bs_from_multi_xy(bs) / 0.5 - 1
            BSS_quant = self.get_quantized_bs_from_multi_xy(bs)
            BSS = BSS + (BSS_quant - BSS).detach()
            cond[:, 1:2] = BSS
            img = img.detach()
            # if path:
            #     import torchvision as tv
            #     tv.utils.save_image(img[0].cpu() / 2 + 0.5, path + f"_step({i})_xt.png")
            s = torch.full((batch,), time_step, device=device)
            if i == time_steps.shape[0] - 1:
                s = cur_time
            if cond is not None:
            ...
            loss = (
                nn.MSELoss(reduction="sum")(torch.exp(2 * ((mask_y * x0 * 0.5 + 0.5))), torch.exp(2 * ((y * 0.5 + 0.5)))) +
                nn.MSELoss(reduction="sum")(torch.exp(2 * ((1 - mask_y * x0 * 0.5 - 0.5))), torch.exp(2 * ((1 - y * 0.5 - 0.5)))) ) / 2 / mask_y.sum()
            ...
            if path:
                ...
                if i % 10 == 0:
                    png_name = ''
                    for j in range(bs[0].shape[0]):
                        png_name += f"{bs[0][j, 0]:.2f}_{bs[0][j, 1]:.2f}-"
                    tv.utils.save_image(x0.cpu()/2+0.5, path + f"_step({i})_x0_{png_name}.png")

            if loss < loss_min:
                bs_best = bs.clone().detach()

            bsgrad = bs.grad
            bs.requires_grad = False
            bsgrad_scale = bsgrad * lr
            bsgrad_step = bsgrad_scale.pow(2).sum(-1).sqrt()
            base = 10
            ...
            for i in range(bs.shape[1]):
                if bs[0, i].min() < 0.0 or bs[0, i].max() >= 256:
                    bs[0, i] = bs_start[0, i]
                if cond[0, 0, torch.floor(bs[0, i, 0]).long(), torch.floor(bs[0, i, 1]).long()] > 0:
                    flag_nobuilding = False
                    if torch.sqrt(BS_m[0, i].pow(2).sum()) < 1:
                        bs[0, i] = bs_isnotbuilding[0, i]
...
class LatentDiffusion(DDPM):
    ...
    @torch.no_grad()
    def sample_given_y(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True, y=None, mask_y=None):
        # image_size, channels = self.image_size, self.channels
        channels = self.channels
        image_size = cond.shape[-2:]
        if cond is not None:
        ...
        if self.cfg.model_name == 'cond_unet8' or self.cfg.model_name == 'cond_unet13':
            ...
            z = self.sample_fn_given_y((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                               up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise, y=y, mask_y=mask_y)
            aux_out = None
    ...
    def sample_fn_given_y(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, y=None, mask_y=None):
        ...
        for i, time_step in enumerate(time_steps):
            ...
            ##############
            if self.scale_by_std:
                z0 = 1. / self.scale_factor * x0.detach()
            elif self.scale_by_softsign:
                z0 = x0 / (1 - x0.abs())
                z0 = z0.detach()

            x_rec = self.first_stage_model.decode(z0)
            xx0 = mask_y.to(x0.device) * y.to(x0.device) + (torch.ones_like(mask_y, dtype=x0.dtype, device=x0.device) - mask_y.to(x0.device)) * x_rec
            z = self.first_stage_model.encode(xx0)
            # x0 = z.mean.to(x0.device)
            x0 = self.get_first_stage_encoding(z)
            if self.scale_by_softsign:
                x0 = F.softsign(x0)
            elif self.scale_by_std:
                x0 = self.scale_factor * x0
