...
class RadioUNet_c_multiTx(Dataset):
    """RadioMapSeer Loader for accurate buildings and no measurements (RadioUNet_c)"""
    def __init__(self,maps_inds=np.zeros(1), phase="train",
                 ind1=0,ind2=0, 
                 dir_dataset="./RadioMapSeer/",
    ...
    def __getitem__(self, idx):
        ...
        if seed < 1/2:
            ...
            # num_Tx = 2 if seed < 2/3 else 3
            num_Tx = 2
            idxr=np.floor(idx/self.numTx).astype(int)
            idxc=idx-idxr*self.numTx 
            while 1:
                sample_Tx = torch.randint(0, 80, (num_Tx-1,))
                if idxc not in sample_Tx:
                    sample_Tx = sample_Tx.tolist()
                    sample_Tx.append(idxc)
                    # print(sample_Tx)
                    break
            dataset_map_ind=self.maps_inds[idxr+self.ind1]+1
            #names of files that depend only on the map:
            name1 = str(dataset_map_ind) + ".png"
            #names of files that depend on the map and the Tx:
            name2 = str(dataset_map_ind) + "_" + '_'.join([str(idxc)] + list(map(str, sample_Tx[:-1]))) + ".png"
            ...
            image_Tx = np.zeros_like(image_buildings, dtype=np.uint8)
            image_gain = []
            for Tx in sample_Tx:
                #Load Tx (transmitter):
                img_name_Tx = os.path.join(self.dir_Tx, str(dataset_map_ind) + "_" + str(Tx) + ".png")
                image_Tx_temp = np.asarray(io.imread(img_name_Tx))
                image_Tx += image_Tx_temp
            
                #Load radio map:
                if self.simulation!="rand":
                    img_name_gain = os.path.join(self.dir_gain, str(dataset_map_ind) + "_" + str(Tx) + ".png")  
                    image_gain_temp = np.expand_dims(np.asarray(io.imread(img_name_gain)),axis=2)/255
                else: #random weighted average of DPM and IRT2
                    img_name_gainDPM = os.path.join(self.dir_gainDPM, str(dataset_map_ind) + "_" + str(Tx) + ".png") 
                    img_name_gainIRT2 = os.path.join(self.dir_gainIRT2, str(dataset_map_ind) + "_" + str(Tx) + ".png") 
                    #image_gainDPM = np.expand_dims(np.asarray(io.imread(img_name_gainDPM)),axis=2)/255
                    #image_gainIRT2 = np.expand_dims(np.asarray(io.imread(img_name_gainIRT2)),axis=2)/255
                    w=np.random.uniform(0,self.IRT2maxW) # IRT2 weight of random average
                    image_gain_temp= w*np.expand_dims(np.asarray(io.imread(img_name_gainIRT2)),axis=2)/256  \
                                + (1-w)*np.expand_dims(np.asarray(io.imread(img_name_gainDPM)),axis=2)/256
                image_gain.append(image_gain_temp)
            

            image_gain = np.concatenate(image_gain, axis=-1)
            image_gain_amp = (image_gain * 100 - 147) / 10
            image_gain = (np.log10(np.sum(10**image_gain_amp, axis=-1)) * 10 + 147) / 100
            image_gain = (image_gain - image_gain.min()) / (image_gain.max() - image_gain.min())
